...
def repeat_last_month(arr, forecast_horizon):
    """Repeat last month's data for forecasting"""
    period = 30
...
# --- Forecast Generation ---
def generate_forecast(model, start_values, fourier_data, steps, params, exog_cols, exog_forecast_scaled, scaler_target):
...
def plot_results(historical_dates, historical_data, forecast_dates, forecast_values, steps, output_dir, timestamp):
    ...
    plt.title(f'{steps}-Day Wind Power Forecast')
    plt.xlabel('Date')
    ...
    plot_filename = f'wind_dhr_daily_{timestamp}_{steps}.png'
...
def run_forecast(csv_path, steps, output_dir="forecasts", forecast_type="daily", params=None):
    """
    Main function to run the DHR model for daily wind power forecasting
    ...
    try:
        ...
        X, y, scaler_target, scaler_exog = create_features(
            ...
            ar_order=params['ar_order'],
            window=params['window'],  # Fixed: Changed 'window_size' to 'window'
            polyorder=params['polyorder']
        )
