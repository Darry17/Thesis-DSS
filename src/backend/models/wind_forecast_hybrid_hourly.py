...
def generate_multi_horizon_forecast(df, dhr_model, esn_model, esn_scaler_target, esn_scaler_exog, best_weights, dhr_params, lags=24, exog_cols=['Wind Speed', 'Temperature', 'Relative Humidity', 'Solar Zenith Angle']):
    """Generate multi-horizon forecasts (1, 24, 168 steps) using last week's exog values"""
    horizons = [1, 24, 168]
    target_col = 'wind_power'
    target = df[target_col].values
    exog_data = df[exog_cols].values
    ...
    for horizon in horizons:
        ...
        week_ago_idx = len(df) - 168 - 1
        if week_ago_idx < 0:
            print(f"Warning: Not enough data for {horizon}-step forecast. Using last available exog values.")
        ...
        for h in range(horizon):
            ...
            dhr_features = np.concatenate([ar_features, smoothed, fourier_forecast[h], exog_forecast[h]])
            dhr_features = dhr_features.reshape(1, -1)
            print(f"DHR features shape for horizon {h+1}: {dhr_features.shape}")
            dhr_pred = dhr_model.predict(dhr_features)[0]
            dhr_pred = max(dhr_pred, 0)
            dhr_forecast.append(dhr_pred)
            current_target = np.append(current_target, dhr_pred)
            ...
            # ESN FORECASTING
            esn_pred = esn_model.run(last_sequence.reshape(1, lags * 5))  # Reshape to [1, lags * n_features]
            esn_pred = np.array(esn_pred).reshape(-1, 1)
            esn_pred_value = esn_pred[0, 0]
            esn_pred_inv = esn_scaler_target.inverse_transform(esn_pred)
            esn_pred_original = np.maximum(np.expm1(esn_pred_inv)[0][0], 0)
        ...
        # Save forecast
        forecast_df.to_csv(f'hybrid_wind_power_forecast_{horizon}h.csv', index=False)
        print(f"Forecast saved to 'hybrid_wind_power_forecast_{horizon}h.csv'")
        ...
        plt.axvline(x=df.index[-1], color='black', linestyle=':', label='Forecast Start', alpha=0.7)
        plt.title(f'{horizon}-Hour Wind Power Forecast with Historical Data', fontsize=16)
        plt.xlabel('Date', fontsize=12)
        plt.ylabel('Wind Power', fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.legend(loc='best')
        plt.gcf().autofmt_xdate()
        plt.tight_layout()
        plt.savefig(f'hybrid_forecast_{horizon}h.png', dpi=300, bbox_inches='tight')
        plt.show()
