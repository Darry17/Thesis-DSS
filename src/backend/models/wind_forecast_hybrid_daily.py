...
def evaluate_security(input_errors, file_errors):
    ...
    file_ops = 5  # pd.read_csv, to_csv (generate_forecast), savefig (generate_forecast), to_csv (multi_horizon), savefig (multi_horizon)
    safe_file_ops = 5 if file_errors.get("file_errors", 0) == 0 else 3
...
def plot_results(dates, actual, dhr_preds, esn_preds, hybrid_preds, steps, output_dir, timestamp, horizon=None):
    ...
    plt.plot(dates[:len(actual)], actual, label='Actual (Last 60 Days)', color='black', linewidth=2)
    ...
    title = f'{steps if horizon is None else horizon}-Day Wind Power Forecast with Historical Data'
    ...
    plot_filename = f'wind_hybrid_daily_{timestamp}_{steps if horizon is None else horizon}d.png'
...
def generate_forecast(df, dhr_model, esn_model, esn_scaler_target, esn_scaler_exog, best_weights, steps, 
    ...
    logging.info(f"Generating {steps}-day forecast...")
    forecast_dates = pd.date_range(start=df.index[-1] + timedelta(days=1), periods=steps, freq='D')
    
    # Get last month's exog values (30 days ago)
    month_ago_idx = len(df) - 30 - 1
    if month_ago_idx < 0:
        logging.warning(f"Not enough data for {steps}-day forecast. Using last available exog values.")
    ...
    if two_months_ago_idx < 0:
        logging.warning(f"Not enough data for two months. Using available actual data.")
...
def generate_multi_horizon_forecast(df, dhr_model, esn_model, esn_scaler_target, esn_scaler_exog, best_weights, dhr_params, lags=7, exog_cols=['Wind Speed', 'Temperature', 'Relative Humidity'], output_dir="forecasts", timestamp=""):
    ...
    for horizon in horizons:
        logging.info(f"Generating {horizon}-day forecast...")
        forecast_dates = pd.date_range(start=df.index[-1] + timedelta(days=1), periods=horizon, freq='D')
        
        # Get last month's exog values (30 days ago)
        month_ago_idx = len(df) - 30 - 1
        if month_ago_idx < 0:
            logging.warning(f"Not enough data for {horizon}-day forecast. Using last available exog values.")
        ...
        # Save forecast
        csv_filename = f'hybrid_wind_power_forecast_{horizon}d_{timestamp}.csv'
...
def train_models(df, target_col, exog_cols, params=None):
    ...
    if len(X_val_dhr) > 0:
        ...
        evaluate_model(y_val_dhr, esn_val_preds)
        logging.info("Optimizing weights...")
        best_weights = optimize_weights(dhr_val_preds, esn_val_preds, y_val_dhr)
        hybrid_val_preds = weighted_average(dhr_val_preds, esn_val_preds, best_weights)
        logging.info("Evaluating Hybrid model on validation set...")
        evaluate_model(y_val_dhr, hybrid_val_preds)
    ...
    if len(X_test_dhr) > 0:
        ...
        evaluate_model(y_test_dhr, esn_test_preds)
        logging.info("Evaluating Hybrid model on test set...")
        hybrid_test_preds = weighted_average(dhr_test_preds, esn_test_preds, best_weights)
        evaluate_model(y_test_dhr, hybrid_test_preds)
