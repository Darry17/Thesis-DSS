...
def load_data(data_path):
    ...
    try:
        ...
        df_resampled = df[required_cols].resample('h').mean().fillna(method='ffill').fillna(method='bfill')
...
def plot_results(historical_dates, historical_data, forecast_dates, forecast_values, steps, output_dir, timestamp):
    """Plot historical and forecast data with seamless connection."""
    ...
    plt.title(f'{steps}-Hour ESN Wind Power Forecast')
    plt.xlabel('Time')
    ...
    plot_filename = f'wind_esn_hourly_{timestamp}_{steps}.png'
...
def run_forecast(csv_path, steps, output_dir="forecasts", forecast_type="hourly", params=None):
    """
    Run the ESN model for hourly wind power forecasting.
    ...
    try:
        ...
        n_features_per_timestep = 1 + len(exog_cols)  # 1 target + 4 exog
        ...
        if len(X_test) > 0:
            ...
            if y_test_pred.shape != y_test.shape:
                logging.warning(f"Reshaping test predictions from {y_test_pred.shape} to {y_test.shape}")
                y_test_pred = y_test_pred.reshape(y_test.shape)
            evaluate_model(y_test, y_test_pred, scaler_target)
        
        # Prepare historical data
        historical_period = 336  # 2 weeks (14 days * 24 hours)
        exog_history_period = 168  # 1 week (7 days * 24 hours)
