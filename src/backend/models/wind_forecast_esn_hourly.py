...
def run_forecast(csv_path, steps, output_dir="forecasts", forecast_type="hourly", params=None):
    ...
    exog_columns = ['Wind Speed', 'Temperature', 'Relative Humidity', 'Solar Zenith Angle']
    
    # Verify exogenous columns exist
    ...
    # Handle missing target variable 'wind_power'
    ...
    data = df_resampled['wind_power'].values.astype(float)
    ...
    # Scaling the target (wind_power) and exogenous variables
    ...
    if len(X_test) > 0:
        ...
        test_plot_path = os.path.join(output_dir, f"wind_test_results_{timestamp}.png")
        plt.figure(figsize=(15, 6))
        plt.plot(y_test_true_original, label='Actual Wind Power', color='blue')
        plt.plot(y_test_pred_original, label='Predicted Wind Power', color='red')
        plt.title('ESN Model: Actual vs Predicted Wind Power (Test Set)')
    ...
    if plot_with_dates:
        plt.plot(historical_dates, historical_data_original, label='Historical Wind Power', color='blue', linewidth=2)
        ...
        plt.plot(range(historical_period), historical_data_original, label='Historical Wind Power', color='blue', linewidth=2)
    ...
    plt.title(f'ESN Wind Power Forecast - {steps} Hour{"s" if steps > 1 else ""}')
    plt.xlabel('Time')
    plt.ylabel('Wind Power')
    plt.legend()
