...
def run_forecast(csv_path, steps, output_dir="forecasts", forecast_type="daily", params=None):
    ...
    exog_columns = ['Wind Speed', 'Temperature', 'Relative Humidity']
    ...
    if df_resampled['wind_power'].isna().any():
        print("Warning: Still have NaN values in 'wind_power' after filling. Removing them.")
    ...
    if plot_with_dates:
        ...
        plt.plot(historical_dates, historical_data_original, label='Historical Daily Wind Power', color='blue', linewidth=2)
        ...
        plt.plot(x_historical, historical_data_original, label='Historical Daily Wind Power', color='blue', linewidth=2)
    ...
    plt.title(f'ESN Daily Wind Power Forecast - {steps} Day{"s" if steps > 1 else ""}')
    plt.xlabel("Date")
    ...
    plot_name = f"wind_esn_daily_{timestamp}_{steps}.png"
