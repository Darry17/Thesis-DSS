...
def evaluate_security(input_errors, file_errors):
    ...
    file_ops = 4  # pd.read_csv, to_csv, plt.savefig (x2)
    safe_file_ops = 4 if file_errors.get("file_errors", 0) == 0 else 3
...
def run_forecast(csv_path, steps, output_dir="forecasts", forecast_type="daily", params=None):
    """
    Main function to run the hybrid DHR-ESN daily solar power forecasting process
    ...
    try:
        ...
        logging.info(f"Enhanced plot with historical data saved to '{plot_path}'")
    except Exception as e:
